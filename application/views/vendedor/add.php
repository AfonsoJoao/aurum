<section class="invoice">
<!-- title row -->
  <div class="row">
    <div class="col-xs-12">
      <h2 class="page-header">
        <i class="fa fa-globe"></i> Cadastrar Vendedor
      </h2>
    </div>
  </div>
  <form>
    <div class="row invoice-info">
      <div class="col-sm-6 invoice-col">
        <b>Informações Pessoais</b><br><br>
        <div class="input-group">
          <span class="input-group-addon">
            Nome
          </span>
          <input type="text" class="form-control" name="nomeVendedor">
        </div><br>

        <div class="input-group">
          <span class="input-group-addon">
            CPF
          </span>
          <input type="text" class="form-control" name="cpfVendedor">
        </div><br>
      </div><br><br>
      <div class="col-sm-6 invoice-col">
        <div class="input-group">
          <span class="input-group-addon">
            Telefone
          </span>
          <input type="text" class="form-control" name="telefoneVendedor">
        </div><br>  
        <div class="input-group">
          <span class="input-group-addon">
            RG
          </span>
          <input type="text" class="form-control" name="rgVendedor">
        </div><br><br>   
      </div>
    </div>      
  </form>
  <div class="row invoice-info">
    <div class="col-sm-6">
      <b>Endereço</b><br><br>
      <div class="input-group">
        <span class="input-group-addon">
          Rua
        </span>
        <input type="text" class="form-control">
      </div><br>
      <div class="input-group">
        <span class="input-group-addon">
          Cidade
        </span>
       <input type="text" class="form-control">
      </div><br>
    </div><br><br>
    <div class="col-sm-2 invoice-col">
      <div class="input-group">
        <span class="input-group-addon">
          N°
        </span>
        <input type="text" class="form-control">
      </div><br>
      <div class="input-group">
        <div class="form-group">
          <select class="form-control" id="sel1">
            <option>AC</option>
            <option>AL</option>
            <option>AP</option>
            <option>AM</option>
            <option>BA</option>
            <option>CE</option>
            <option>DF</option>
            <option>ES</option>
            <option>AM</option>
            <option>AM</option>
            <option>AM</option>
          </select>
        </div>
      </div><br>          
    </div>
    <div class="col-sm-4 invoice-col">
      <div class="input-group">
        <span class="input-group-addon">
          Bairro
        </span>
        <input type="text" class="form-control">
      </div><br>
      <div class="input-group">
        <span class="input-group-addon">
          Complemento
        </span>
        <input type="text" class="form-control">
      </div><br>
    </div>
  </div><br>
<div class="row no-print">
<div class="col-xs-12">
<button type="button" class="btn btn-success pull-right"><i class="fa fa-chevron-down"></i> Cadastrar
<button type="button" class="btn btn-warning pull-right" style="margin-right: 5px;">
<i class="fa fa-eraser"></i> Limpar Campos
</button>
</div>
</div>
</section>                